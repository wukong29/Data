 ...
 */
func (bc BlockChain) QuerBlockByHeight(height int64)*Block{
	if height < 0 {//如果目标高度小于零，则说明参数不合法
		return nil
	}
	var block *Block
	db := bc.BoltDb
	db.View(func(tx *bolt.Tx) error {
		...
		hashKey := bc.LastHash
		for{
			lastBlockByres :=bucket.Get(hashKey)
			eachBlock,_ := DeSerialize(lastBlockByres)
			if eachBlock.Height < height {//查询给定的数字超出区块链中的高度，直接返回
				break
			}
